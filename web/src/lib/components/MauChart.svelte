<script lang="ts">
  ...
  function sliceByDays<T extends { day: string }>(rows: T[], days: number): T[] {
    ...
    const cutoff = new Date(Date.UTC(+max.slice(0, 4), +max.slice(5, 7) - 1, +max.slice(8, 10)) - days * 86400000)
  ...
  const lines = $derived<ChartLine[]>([
    { name: 'MAD (28d)', color: 'var(--amber)', data: wWindowed.map((d) => ({ day: d.day, value: d.all_devs })) },
    { name: 'single-chain', color: 'var(--cyan)', data: wWindowed.map((d) => ({ day: d.day, value: d.exclusive_devs })) },
    { name: 'multi-chain', color: 'var(--rose)', data: wWindowed.map((d) => ({ day: d.day, value: d.multichain_devs })) },
    ...(wApi.length ? [{ name: 'API total', color: 'var(--amber-soft)', dash: '4 3',
        data: wApi.map((d) => ({ day: d.day, value: d.total as number })) }] : []),
    ...(wCohort.length ? [{ name: 'recurring base', color: '#7d8aa8', dash: '2 3',
        data: wCohort.map((d) => ({ day: d.day, value: d.recurring })) }] : [])
  ]);
  const bars = $derived<ChartBars | null>({ name: 'daily active', color: 'var(--amber)',
