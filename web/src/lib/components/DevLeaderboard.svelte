<script lang="ts">
    ...
    const rows = $derived.by(() => {
        const pick = (d: DevAgg) =>
            win === 28
                ? { commits: d.c28, days: d.a28, repos: d.r28 }
                : win === 60
                  ? { commits: d.c60, days: d.a60, repos: d.r60 }
                  : { commits: d.c90, days: d.a90, repos: d.r90 };
        return devs
            .map((d) => ({ name: d.name, login: d.login, ...pick(d) }))
...
<section class="panel chartwrap">
    <div class="chart-head">
        ...
        <div class="toggle">
            {#each [28, 60, 90] as d (d)}<button class:active={win === d} onclick={() => (win = d)}
                    >{d}d</button
                >{/each}
            <span class="div"></span>
    ...
    {#if devs.length === 0}
        <p class="note">
            Run <em>stellar_odd.py resolve-devs</em> to populate developer identities.
        </p>
    {:else}
        <table>
            <thead
